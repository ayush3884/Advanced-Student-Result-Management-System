...
class CourseClass:
    def __init__(self, root):
        ...
        #=====Search Button ====
        txt_Search_CourseName=Button(self.root,text="Search",font=("goudy old style",15,"bold"),bg="#03a9f4",fg="white",cursor="hand2",command=self.search).place(x=1070,y=60,width=120,height=28)
        ...
        try:
            if self.var_course.get()=="":
                ...
                if row!=None:
                    ...
                    con.commit()
                    messagebox.showinfo("Succss","Course Added Successfully",parent=self.root)
                    self.show()
        except Exception as ex:
             messagebox.showerror("Error",f"Error due to {str(ex)}")
        ...
        try:
            if self.var_course.get()=="":
                ...
                if row==None:
                    ...
                    con.commit()
                    messagebox.showinfo("Succss","Course Update Successfully",parent=self.root)
                    self.show()
        except Exception as ex:
             messagebox.showerror("Error",f"Error due to {str(ex)}")
